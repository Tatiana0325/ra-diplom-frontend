...
export const fetchCatalogCategoriesEpics = (action$) =>
  action$.pipe(
    ofType(FETCH_CATALOG_CATEGORIES_REQUEST),
    map(() => store.getState().catalogList),
    switchMap(() =>
      ajax.getJSON(`${process.env.REACT_APP_URL}categories`).pipe(
...
export const fetchCatalogItemsEpics = (action$) =>
  action$.pipe(
    ...
    map(() => {
      ...
      const queryUrl = queryString.stringify(
        { q: search, categoryId: activCategory, offset: items.length },
        {
    ...
    switchMap((query) =>
      ajax.getJSON(`${process.env.REACT_APP_URL}items${query}`).pipe(
...
export const fetchCatalogCategoriesChangeEpics = (action$) =>
  action$.pipe(
    ofType(FETCH_CATALOG_CATEGORIES_CHANGE),
    map(() => fetchCatalogItemsRequest())
  );

export const searchChangeEpics = (action$) =>
  action$.pipe(
